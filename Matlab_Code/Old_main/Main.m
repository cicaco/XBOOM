...
phi_vect=linspace(0,90,100).*pi./180;
for jj=1:100
    jj
    phi=phi_vect(jj); %Portandolo a 90 si tende a stringere il cerchio
    ...
    [TOUT,YOUT] = ode15s(@(t,y)EquationOfMotions(t,y,fileID,BoomInfo,Tl_0),[0 tfin],Y0,options); %
    fclose(fileID);
    
   Dist(jj)=norm(YOUT(end,10:12));
   FIN(jj)=TOUT(end);
   Z(jj)=YOUT(end,12);
end
%save('Phi0_90.mat','Ris');
%%
figure()
