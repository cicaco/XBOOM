...
beta=5*pi/180; %Angolo di Diedro
...
axis equal
hold on
plot(C_fin(1,num-p_c),C_fin(2,num-p_c),'*g');
plot(C_fin(1,:),C_fin(2,:),'ok');

plot(C_fin(1,num+p_c),C_fin(2,num+p_c),'*b');
figure()
C_fin_rot(2,num)=0;
...
quiver3(C_fin_rot(1,num-p_c),C_fin_rot(2,num-p_c),C_fin_rot(3,num-p_c),Norm(1),Norm(2),Norm(3));
[I_dx,rc] = line_plane_intersection(Norm,C_fin_rot(:,num-p_c),[0 1 0]',[0 0 0]');
...
hold on
% plot3(I_origin_Sx(1),I_origin_Sx(2),I_origin_Sx(3),'ok');
% plot3(I_Start_Sx(1),I_Start_Sx(2),I_Start_Sx(3),'*g');
% plot3(I_Finish_Sx(1),I_Finish_Sx(2),I_Finish_Sx(3),'*r');
axis equal
I_origin_Dx=I_dx-CG';
I_Start_Dx=C_fin_rot(:,num+p_c)-CG';
I_Finish_Dx=C_fin_rot(:,end)-CG';
% plot3(I_origin_Dx(1),I_origin_Dx(2),I_origin_Dx(3),'ok');
% plot3(I_Start_Dx(1),I_Start_Dx(2),I_Start_Dx(3),'*g');
% plot3(I_Finish_Dx(1),I_Finish_Dx(2),I_Finish_Dx(3),'*r');
