...
hold on
Chord_R=Plant3D(1,1:100)+(fliplr(Plant3D(1,101:200))-(Plant3D(1,1:100)))./4;
Chord_L=Plant3D(1,1:100)-(fliplr(Plant3D(1,101:200))-(Plant3D(1,1:100))).*3./4;

Aerc=[fliplr(Chord_R) Chord_L;fliplr(Plant3D(2,1:100)) -Plant3D(2,1:100)];
...
Psi=YOUT(:,3)*180/pi;
Theta=YOUT(:,1)*180/pi;
save('TLore.mat','Time','Theta' ,'Phi' ,'Psi' , 'x' ,'y' ,'z');
%%
j=1;
p=YOUT(j,4);
q=YOUT(j,5);
...
uy=YOUT(j,8);
uz=YOUT(j,9);
[F,M,M_body,XY_ac,F_body]= AeroLore(p,q,r,ux,uy,uz,Geom);
%% Aerodynamic
...
M=zeros(3,num);
for i=1:num
[F(:,i),M(:,i)]=AeroLore(p(i),q(i),r(i),ux(i),uy(i),uz(i),Geom);
